...
test("Проверка лэйаута контейнера управления вызова screenshot", async ({
  roomPage,
}) => {
  await roomPage.videoCallControlsHasCorrectAriaSnapshot();
});
...
test(
  "Проверка работы кнопки перехода в полноэкранный режим",
  async ({ roomPage }) => {
...
test("Отправка сообщения и проверка его отображения в чате", async ({
  roomPage,
}) => {
...
test("Проверка индикатора количества участников в комнате", async ({
  roomPage,
}) => {
